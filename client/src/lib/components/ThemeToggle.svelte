<script lang="ts">
  ...
  let currentTheme = $state('elysia-light');
  ...
  onMount(() => {
    ...
    if (savedTheme) {
      ...
      isChecked = savedTheme === 'elysia-dark';
      document.documentElement.setAttribute('data-theme', savedTheme);
    } else {
      // Check system preference
      const prefersDark = window.matchMedia('(prefers-color-scheme: dark)').matches;
      currentTheme = prefersDark ? 'elysia-dark' : 'elysia-light';
  ...
  function handleToggleClick() {
    isChecked = !isChecked;
    currentTheme = isChecked ? 'elysia-dark' : 'elysia-light';
    document.documentElement.setAttribute('data-theme', currentTheme);
    localStorage.setItem('theme', currentTheme);
  }
</script>

<button onclick={handleToggleClick} class="relative btn btn-ghost btn-circle" aria-label="Toggle theme">
  <!-- sun icon -->
  <svg
